...
const navbar = document.querySelector(".navbar"); // Menggunakan .navbar dari HTML Anda

if (menuToggle && navbar) { // Pastikan navbar juga ada
  menuToggle.onclick = () => {
...
function showPage(pageId) {
  ...
  document
    .querySelectorAll(".navbar button") // Menggunakan .navbar dari HTML Anda
...
// "aboutme" DITAMBAHKAN PADA BARIS DI BAWAH INI
["home", "hash", "block", "chain", "ecc", "consensus", "aboutme"].forEach(
  (p) => {
    ...
    if (t) {
      t.onclick = () => {
        ...
        if (navbar) { // Pastikan navbar ada
          navbar.classList.remove("open"); // <-- BARU: Tutup menu saat link diklik
        }
      };
    }
  }
...
async function updateBlockHash() {
  if (!blockData) return; // Pastikan elemen ada
...
function renderChain() {
  ...
  blocks.forEach((blk, i) => {
    ...
    div.innerHTML = `
      ...
      <button onclick="mineChainBlock(${i})" class="btn btn-primary">Mine</button>
...
if (btnAddBlock) {
  ...
  addChainBlock(); // Inisialisasi chain dengan 1 blok
}
...
function parseTx(line) {
  ...
  if (!m) return null;
  // ===============================================
  // INI ADALAH PERBAIKANNYA (m2 -> m[2])
  // ===============================================
...
// ======== Kirim Transaksi ========
["A", "B", "C"].forEach((u) => {
  const btnSend = document.getElementById("send-" + u);
  if (btnSend) {
    btnSend.onclick = () => {
...
// Hanya jalankan jika kita berada di halaman konsensus
